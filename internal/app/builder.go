...
func (s *BuilderService) generateContent(modules []domain.Module, opts BuildOptions, shellType, target string, now time.Time) (content string, moduleNames []string) {
	var lines []string
	...
	lines = append(
		...
		fmt.Sprintf("# Modules: %d", len(modules)),
		fmt.Sprintf("# Generated at: %s", now.Format(time.RFC3339)),
		"",
	)
