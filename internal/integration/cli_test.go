...
func TestCLI_Help(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			expected: []string{
				"Build generates shell configuration files",
				"--manifest",
				"--config-dir",
				"--os",
				"--output-dir",
...
`
	...
	outputDir := filepath.Join(tempDir, "buildout")
	...
	t.Run("successful build", func(t *testing.T) {
		...
		assert.Contains(t, string(output), "Generated", "Should show success message")

		// Verify RC files were generated in the output directory
		entries, err := os.ReadDir(outputDir)
		require.NoError(t, err, "Output directory should exist")
		require.NotEmpty(t, entries, "Output directory should contain generated files")

		// Verify generated content across all RC files
		var combined string
		...
		assert.Contains(t, combined, "TEST_VAR", "Output should contain module content")
		assert.Contains(t, combined, "Generated by shellforge", "Output should have header")
	})
...
func TestCLI_ExitCodes(t *testing.T) {
	...
	tests := []struct {
		...
		{
			// OS is auto-detected, so build has no required flag; with no
			// manifest in the working directory it fails on manifest read.
			name:         "build missing manifest",
			args:         []string{"build", "--manifest", "/nonexistent/manifest.yaml"},
			expectError:  true,
			errorPattern: "manifest",
		},
		{
			name:         "invalid flag",
